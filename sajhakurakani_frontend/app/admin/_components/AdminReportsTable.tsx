"use client";

import { useMemo, useState, useTransition } from "react";
import { useRouter } from "next/navigation";
import { toast } from "react-toastify";
import type { AdminReportItem } from "@/lib/api/admin";

type AdminReportsTableProps = {
  csrfToken: string;
  reports: AdminReportItem[];
};

type ConfirmationState = Record<
  string,
  {
    confirmationId: string;
    expiresAt: string;
  }
>;

const formatDate = (value: string) =>
  new Date(value).toLocaleString("en-US", {
    month: "short",
    day: "numeric",
    year: "numeric",
    hour: "numeric",
    minute: "2-digit",
  });

export default function AdminReportsTable({
  csrfToken,
  reports,
}: AdminReportsTableProps) {
  const router = useRouter();
  const [isPending, startTransition] = useTransition();
  const [reasonById, setReasonById] = useState<Record<string, string>>({});
  const [durationById, setDurationById] = useState<Record<string, string>>({});
  const [confirmations, setConfirmations] = useState<ConfirmationState>({});

  const reportList = useMemo(() => reports, [reports]);

  const runCommand = (params: Record<string, string>) => {
    startTransition(async () => {
      try {
        const formData = new FormData();
        formData.set("_csrf", csrfToken);

        Object.entries(params).forEach(([key, value]) => {
          formData.set(key, value);
        });

        const response = await fetch("/api/admin/command", {
          method: "POST",
          body: formData,
        });

        const payload = (await response.json()) as {
          success: boolean;
          message?: string;
          data?: { confirmationId?: string; expiresAt?: string };
        };

        if (!response.ok || !payload.success) {
          throw new Error(payload.message || "Unable to complete that admin action right now.");
        }

        if (
          params.command === "user.ban.start" &&
          payload.data?.confirmationId &&
          payload.data?.expiresAt
        ) {
          setConfirmations((current) => ({
            ...current,
            [params.id]: {
              confirmationId: payload.data!.confirmationId!,
              expiresAt: payload.data!.expiresAt!,
            },
          }));
          toast.success("Ban confirmation started. Confirm it within 10 seconds.");
          return;
        }

        setConfirmations((current) => {
          const next = { ...current };
          delete next[params.id];
          return next;
        });
        toast.success(payload.message || "Admin action completed.");
        router.refresh();
      } catch (error) {
        toast.error(
          error instanceof Error
            ? error.message
            : "Unable to complete that admin action right now."
        );
      }
    });
  };

  return (
    <div className="space-y-4">
      {reportList.length === 0 ? (
        <article className="rounded-[24px] border border-[#ead6ca] bg-white/92 p-6 shadow-[0_18px_42px_rgba(88,57,38,0.08)]">
          <p className="text-lg font-semibold text-[#1d243f]">No reports found</p>
          <p className="mt-2 text-sm leading-6 text-[#6a7282]">
            There are no matching reports for the current filter.
          </p>
        </article>
      ) : (
        reportList.map((report) => {
          const reason = reasonById[report._id] || "";
          const duration = durationById[report._id] || "24";
          const confirmation = confirmations[report._id];
          const contentTargetId =
            report.type === "post"
              ? report.post?._id
              : report.type === "comment"
              ? report.comment?._id
              : undefined;
          const actionTargetUserId =
            report.type === "friend-request"
              ? report.reportedUser?._id
              : report.type === "post"
              ? report.post?.author?._id
              : report.comment?.author?._id;

          return (
            <article
              key={report._id}
              className="rounded-[24px] border border-[#ead6ca] bg-white/92 p-5 shadow-[0_18px_42px_rgba(88,57,38,0.08)]"
            >
              <div className="flex flex-wrap items-start justify-between gap-3">
                <div>
                  <p className="text-[0.72rem] font-semibold uppercase tracking-[0.26em] text-[#ef744b]">
                    {report.type.replace("-", " ")} report
                  </p>
                  <h2 className="mt-2 text-lg font-semibold text-[#1d243f]">
                    {report.reason || "No report reason provided"}
                  </h2>
                  <p className="mt-2 text-sm leading-6 text-[#6a7282]">
                    Reporter:{" "}
                    <span className="font-medium text-[#1d243f]">
                      {report.reporter?.firstName || "Unknown"} {report.reporter?.lastName || ""}
                    </span>
                    {report.reporter?.username ? ` · @${report.reporter.username}` : ""}
                  </p>
                </div>

                <div className="rounded-[16px] border border-[#efe2d9] bg-[#fffaf6] px-4 py-3 text-right">
                  <p className="text-[0.72rem] uppercase tracking-[0.22em] text-[#8c8690]">
                    Status
                  </p>
                  <p className="mt-2 text-sm font-semibold text-[#1d243f]">
                    {report.status}
                  </p>
                  <p className="mt-1 text-xs text-[#8c8690]">{formatDate(report.createdAt)}</p>
                </div>
              </div>

              {report.description ? (
                <p className="mt-4 rounded-[16px] border border-[#efe2d9] bg-[#fffaf6] px-4 py-3 text-sm leading-6 text-[#6a7282]">
                  {report.description}
                </p>
              ) : null}

              <div className="mt-4 grid gap-3 lg:grid-cols-[1.4fr_140px]">
                <input
                  type="text"
                  value={reason}
                  onChange={(event) =>
                    setReasonById((current) => ({
                      ...current,
                      [report._id]: event.target.value,
                    }))
                  }
                  placeholder="Reason for admin action"
                  className="rounded-[14px] border border-[#ead9ce] bg-[#fffdfa] px-4 py-3 text-sm text-[#1d243f] outline-none placeholder:text-[#ada4ad]"
                />
                <input
                  type="number"
                  min={1}
                  max={720}
                  value={duration}
                  onChange={(event) =>
                    setDurationById((current) => ({
                      ...current,
                      [report._id]: event.target.value,
                    }))
                  }
                  placeholder="Hours"
                  className="rounded-[14px] border border-[#ead9ce] bg-[#fffdfa] px-4 py-3 text-sm text-[#1d243f] outline-none placeholder:text-[#ada4ad]"
                />
              </div>

              <div className="mt-4 flex flex-wrap gap-2">
                <button
                  type="button"
                  disabled={isPending}
                  onClick={() =>
                    runCommand({
                      command: "report.dismiss",
                      id: report._id,
                      reason,
                    })
                  }
                  className="rounded-[12px] border border-[#ead6ca] bg-[#fff8f3] px-4 py-2 text-sm font-semibold text-[#526077]"
                >
                  Dismiss
                </button>
                {actionTargetUserId ? (
                  <>
                    <button
                      type="button"
                      disabled={isPending}
                      onClick={() =>
                        runCommand({
                          command: "report.warn",
                          id: report._id,
                          reason,
                        })
                      }
                      className="rounded-[12px] border border-[#ead6ca] bg-[#fff8f3] px-4 py-2 text-sm font-semibold text-[#526077]"
                    >
                      Warn
                    </button>
                    <button
                      type="button"
                      disabled={isPending}
                      onClick={() =>
                        runCommand({
                          command: "report.suspend",
                          id: report._id,
                          reason,
                          durationHours: duration,
                        })
                      }
                      className="rounded-[12px] border border-[#f1d1b8] bg-[#fff0e6] px-4 py-2 text-sm font-semibold text-[#9c4f2e]"
                    >
                      Suspend
                    </button>
                    {!confirmation ? (
                      <button
                        type="button"
                        disabled={isPending}
                        onClick={() =>
                          runCommand({
                            command: "user.ban.start",
                            id: actionTargetUserId,
                            reason,
                          })
                        }
                        className="rounded-[12px] bg-[linear-gradient(135deg,#f68155_0%,#ef744b_100%)] px-4 py-2 text-sm font-semibold text-white"
                      >
                        Start ban
                      </button>
                    ) : (
                      <button
                        type="button"
                        disabled={isPending}
                        onClick={() =>
                          runCommand({
                            command: "user.ban.confirm",
                            id: actionTargetUserId,
                            reason,
                            confirmationId: confirmation.confirmationId,
                          })
                        }
                        className="rounded-[12px] bg-[#1d243f] px-4 py-2 text-sm font-semibold text-white"
                      >
                        Confirm ban
                      </button>
                    )}
                  </>
                ) : null}
                {contentTargetId && report.type === "post" ? (
                  <>
                    <button
                      type="button"
                      disabled={isPending}
                      onClick={() =>
                        runCommand({
                          command: "post.hide",
                          id: contentTargetId,
                          reason,
                        })
                      }
                      className="rounded-[12px] border border-[#ead6ca] bg-[#fff8f3] px-4 py-2 text-sm font-semibold text-[#526077]"
                    >
                      Hide post
                    </button>
                    <button
                      type="button"
                      disabled={isPending}
                      onClick={() =>
                        runCommand({
                          command: "post.delete",
                          id: contentTargetId,
                          reason,
                        })
                      }
                      className="rounded-[12px] border border-[#f2c3b7] bg-[#fff1ed] px-4 py-2 text-sm font-semibold text-[#b14f3f]"
                    >
                      Delete post
                    </button>
                  </>
                ) : null}
                {contentTargetId && report.type === "comment" ? (
                  <>
                    <button
                      type="button"
                      disabled={isPending}
                      onClick={() =>
                        runCommand({
                          command: "comment.hide",
                          id: contentTargetId,
                          reason,
                        })
                      }
                      className="rounded-[12px] border border-[#ead6ca] bg-[#fff8f3] px-4 py-2 text-sm font-semibold text-[#526077]"
                    >
                      Hide comment
                    </button>
                    <button
                      type="button"
                      disabled={isPending}
                      onClick={() =>
                        runCommand({
                          command: "comment.delete",
                          id: contentTargetId,
                          reason,
                        })
                      }
                      className="rounded-[12px] border border-[#f2c3b7] bg-[#fff1ed] px-4 py-2 text-sm font-semibold text-[#b14f3f]"
                    >
                      Delete comment
                    </button>
                  </>
                ) : null}
              </div>

              {confirmation ? (
                <p className="mt-3 text-xs text-[#8c8690]">
                  Ban confirmation active until {formatDate(confirmation.expiresAt)}.
                </p>
              ) : null}
            </article>
          );
        })
      )}
    </div>
  );
}
